    ...
    <body>
        ...
        <script>
            $(function () {
                ...
                // Utilidades de fecha (local-safe)
                ...
                function todayYMDLocal() {                 // hoy en local 'YYYY-MM-DD'
                    const d = new Date();
                ...
                // Carga proveedores activos en un <select>
                function cargarProveedoresSelect(opts = {}) {
                    const {
                        selectId = 'Proveedor',     // id del <select>
                        selected = '',              // id_proveedor preseleccionado (opcional)
                        incluirTodos = true,        // agrega opción "-- Todos --"
                        limite = 200,               // cuántos traer como máximo
                        q = ''                      // filtro de texto opcional
                    } = opts;
                    ...
                    .fail(function(){
                        // Si falla la carga, dejamos el select habilitado con la opción por defecto
               ...
               // =================== Helpers ===================
                ...
                $.getJSON('<?= BASE_URL ?>/controllers/ProductosController.php', {accion:'buscar-min', q: term, limite: 10})
                    ...
                    if (!items.length){ $sug.html('<div class="p-2 text-muted">Sin resultados</div>').show(); return; }
                    let html = '<div class="list-group list-group-flush">';
                    items.forEach(it=>{
                        const label = `${it.codigo ? it.codigo + ' - ' : ''}${it.descripcion}`;
                        html += `
                        <a href="#" class="list-group-item list-group-item-action ac-pick"
                            data-id="${it.id_producto}" data-costo="${it.costo_sugerido || 0}"
                            title="${label}">${label}</a>`;
                    });
                    html += '</div>';
                    $sug.html(html).show();
                ...
                const cost = Number($a.data('costo')||0);
                ...
                // actualiza estado y precio si vacío
                if (Number.isFinite(idx)){
                    AC_ROWS[idx].id_producto = id;
                    if (!Number($tr.find('.ac-precio').val())) {
                    AC_ROWS[idx].precio_unitario = cost;
                    $tr.find('.ac-precio').val(cost);
                    }
                }

                // recalcula importes
                $tr.find('.ac-precio').trigger('input');
                $cell.find('.ac-sug').hide().empty();
                });

                // cerrar sugerencias si se clickea fuera
                $(document).on('click', function(e){
                if (!$(e.target).closest('.ac-prod-cell').length) {
                    $('.ac-sug').hide().empty();
                }
                });
                ...
                if (AC_ROWS.length === 0) {
                    ...
                    $tb.append(`
                        ...
                        <td><input type="number" class="form-control form-control-sm ac-precio" min="0" step="0.0001" value="${r.precio_unitario||''}" placeholder="0.00"></td>
                ...
                const idpHidden = Number($tr.find('.ac-idp-hidden').val()); // viene del pick
                ...
                AC_ROWS[idx].precio_unitario = isFinite(precio) ? precio : 0;
                ...
                // agregar/limpiar filas (igual que ya tenías)
                ...
                $('#ac-btn-guardar').on('click', function(){
                    ...
                    const id_proveedor = Number($('#ac-proveedor').val());   // << ahora del select
                    ...
                    if (!filasValidas.length){
                        $('#ac-error').show().text('Agrega al menos un renglón válido (producto, cantidad y costo).');
                        return;
                    }
