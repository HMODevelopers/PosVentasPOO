...
<body>
              ...
              <div class="col-md-4">
                <div class="form-group">
                  ...
                  <div class="input-group">
                    <input type="text" id="FiltroNombre" class="form-control filtrar" placeholder="Administrador, Vendedor...">
        ...
        <div class="col-12">
          <div class="card-box">
            <div class="d-flex justify-content-between align-items-center mb-2">
              ...
              <button id="btnNuevo" class="btn btn-primary"><i class="mdi mdi-plus"></i> Nuevo rol</button>
            </div>
            ...
            <div class="row align-items-center justify-content-between mt-2">
              <div class="col-md-6">
                <div id="infoRoles" class="dataTables_info" role="status" aria-live="polite"></div>
              </div>
  ...
  <div class="modal fade" id="modalEliminar" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <p class="mb-0">¿Seguro que deseas eliminar el rol <strong id="delNombre"></strong>? Esta acción es reversible.</p>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-light" data-dismiss="modal">Cancelar</button>
  ...
  <script>
    $(function(){
      ...
      const URL_CTRL = '<?= BASE_URL ?>/controllers/RolesController.php';
      ...
      $(".filtrar")
        .change(function(){
          ...
          $el.blur();
          setTimeout(()=> cargarRoles(1), 200);
        })
        .keypress(function(e){ if (e.charCode == 13) cargarRoles(1); })
        .keyup(function(){
      ...
      function cargarRoles(pagina){
        ...
        .done(function(resp){
          ...
          if (total === 0) {
            $('#infoRoles').text('No hay roles para mostrar');
          } else {
            const desde = (pagina - 1) * limitePorPagina + 1;
            const hasta = Math.min(pagina * limitePorPagina, total);
            $('#infoRoles').text(`Mostrando ${desde} a ${hasta} de ${total} roles`);
          }
      ...
      function renderizarTabla(rows){
        ...
        if (!rows.length){
          ...
          tbody = '<tr><td colspan="6" class="text-center text-muted">— No hay registros —</td></tr>';
        } else {
      ...
      function configurarPaginacion(currentPage, totalItems, itemsPerPage=10)
      {
        ...
        $ul.off('click','a.page-link').on('click','a.page-link', function(e){
          ...
          if (Number.isFinite(page)) { paginaActual = page; cargarRoles(paginaActual); }
        });
      }
      ...
      $(document).on('click', 'a.accion-editar', function(e){
        ...
        $.ajax({
          ...
          data: { accion: 'detalle', id_rol: id }
        })
        .done(function(resp){
          const r = resp?.data || null;
          if (!r){ toastr.error('No se encontró el rol.'); return; }
          $('#id_rol').val(r.id_rol);
          $('#nombre').val(r.nombre || '');
      ...
      $('#formRol').on('submit', function(e){
        ...
        .done(function(resp){
          const ok = !!resp?.ok || (accion==='crear' && resp?.id_rol>0);
          if (ok){
            $('#modalRol').modal('hide');
            toastr.success('Rol guardado correctamente.');
            cargarRoles(accion==='crear' ? 1 : paginaActual);
          } else {
