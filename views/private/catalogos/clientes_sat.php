...
<div class="col-12 col-md-6"><div class="form-group"><label>Entidad</label><select id="estado" class="form-control"></select><div class="fallback-wrap mt-2" id="estado_fallback_wrap"><input id="estado_fallback" class="form-control" placeholder="Texto legado de estado"></div></div></div>
<div class="col-12 col-md-6"><div class="form-group"><label>Municipio</label><select id="municipio" class="form-control"></select><div class="fallback-wrap mt-2" id="municipio_fallback_wrap"><input id="municipio_fallback" class="form-control" placeholder="Texto legado de municipio"></div></div></div>
<div class="col-12 col-md-6"><div class="form-group"><label>Localidad</label><select id="localidad" class="form-control"></select><div class="fallback-wrap mt-2" id="localidad_fallback_wrap"><input id="localidad_fallback" class="form-control" placeholder="Texto legado de localidad"></div></div></div>
...
$(function(){
  ...
  function cargarCatalogos(){ return $.getJSON(URL_CTRL,{accion:'catalogos_form'}).then(resp=>{catalogos=resp||catalogos;fillSimple('#regimen_fiscal',catalogos.regimenes||[],'ClaveRegimenFiscal','Descripcion');fillSimple('#uso_cdfi',catalogos.usos_cfdi||[],'ClaveUsoCFDI','Descripcion');fillSimple('#estado',catalogos.entidades||[],'cvegeo','nombre_ent');$('#municipio').html('<option value="">Seleccione entidad...</option>');$('#localidad').html('<option value="">Seleccione municipio...</option>');}); }

  function cargarMunicipios(cveEnt,selected=''){ if(!cveEnt){$('#municipio').html('<option value="">Seleccione entidad...</option>').trigger('change.select2'); return $.Deferred().resolve().promise();}
  ...
  function cargarLocalidades(cveEnt,cveMun,selected=''){ if(!cveEnt||!cveMun){$('#localidad').html('<option value="">Seleccione municipio...</option>').trigger('change.select2'); return $.Deferred().resolve().promise();}
  ...
  $('#estado').on('change',function(){ const cveEnt=$(this).val(); cargarMunicipios(cveEnt); cargarLocalidades('',''); });
  $('#municipio').on('change',function(){ const mun=$(this).val(); const ent=(mun||'').toString().slice(0,2)||$('#estado').val(); cargarLocalidades(ent,(mun||'').toString().slice(2,5)); });
  ...
  $(document).on('click','a.accion-editar',function(ev){ev.preventDefault();
    $.getJSON(URL_CTRL,{accion:'detalle',id:$(this).data('id'),rfc:$(this).data('rfc'),row_key:$(this).data('row')},async resp=>{
      ...
      const estadoSel=r.estado_select||''; const municipioSel=r.municipio_select||''; const localidadSel=r.localidad_select||'';
      $('#estado').val(estadoSel).trigger('change.select2'); await cargarMunicipios((estadoSel||'').slice(0,2),municipioSel); await cargarLocalidades((municipioSel||estadoSel||'').slice(0,2),(municipioSel||'').slice(2,5),localidadSel);
      toggleFallback('estado',r.estado_texto_fallback||''); toggleFallback('municipio',r.municipio_texto_fallback||''); toggleFallback('localidad',r.localidad_texto_fallback||'');
      $('#tituloModal').text('Editar cliente SAT');$('#modalForm').modal('show');
    });
  });

  $('#formRegistro').submit(function(ev){ev.preventDefault();const payload={};
    $(this).find('input,select').each(function(){if(this.id && !this.id.endsWith('_fallback'))payload[this.id]=$(this).val();});
    ['estado','municipio','localidad'].forEach(k=>{ if(!payload[k]){ const fb=$(`#${k}_fallback`).val(); if((fb||'').trim()!=='') payload[k]=fb; } });
